...
- `DESCRIBE` has `IRI` as an argument and returns all predicates and objects the subject has.
...
- `spq_add("?s ?p ?p")` adds a triple pattern to match. 
...
- `spq_head(n = <n>)` limits number of returned results using `LIMIT n`. 
